...
const Map = () => {
  ...
  useEffect(() => {
    ...
    svg.selectAll('path')
      ...
      .on('mouseover', (event, d) => {
        ...
        tooltip.style('visibility', 'visible')
          .text(d.properties.name); // Display the country's name
      })
      .on('mousemove', (event) => {
    ...
    // Function to render the Self location marker
    const renderSelfMarker = (selfLocation) => {
      const [x, y] = projection([selfLocation.longitude, selfLocation.latitude]);

      const selfMarker = svg.append('circle')
        .attr('cx', x)
        .attr('cy', y)
        .attr('r', 8) // ขนาดของ marker
        .attr('fill', 'blue') // สีของ marker
        .attr('stroke', '#fff')
        .attr('stroke-width', 2);
    ...
    fetch('https://ipinfo.io/json')
      ...
      .then((data) => {
        ...
        renderSelfMarker(selfData); // เรียกฟังก์ชันเพื่อแสดงตำแหน่งตัวเอง

        // Continue with existing attacker rendering
